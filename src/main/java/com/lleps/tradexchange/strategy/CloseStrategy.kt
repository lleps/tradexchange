...
class CloseStrategy(val cfg: Config, val timeSeries: TimeSeries, val buyTick: Int, val buyPrice: Double) {
    ...
    private val close = ClosePriceIndicator(timeSeries)
    private val emaLong = EMAIndicator(close, cfg.longEmaPeriod) // To detect market trend change
    ...
    class Tester : Application() {
        ...
        private fun execute(cfg: Config, retry: Boolean = false) {
            if (!retry) {
                lastTickInitial = Random.nextInt(series.barCount)
