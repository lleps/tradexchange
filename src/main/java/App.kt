...
class App : Application() {
    ...
    private fun handle(period: Long, history: TimeSeries, end: Int) {
        ...
        val percent = obvOscillatorIndicator[end] * macdIndicator[end]
        ...
        } else if (exchange.moneyBalance >= price && percent < 0.2 && farBelowLastBuy) {
            val nextEventId = (state.events.map { it.id }.max() ?: 0) + 1
        ...
        chart.addPointExtra("BOTH", "both", epoch, macdIndicator[end] * obvOscillatorIndicator[end])
